 ...
 */
async function enrichOldSpecType(
  oldSpecTypeName: string,
  oldUnit: string,
  oldValueType: string,
  oldDescription: string
): Promise<EnrichedOldSpecType> {
...
Return ONLY a JSON object:
{
  ...
  return await askClaudeJSON<EnrichedOldSpecType>(prompt, systemPrompt);
}
...
async function convertOldSpecTypes() {
  ...
  const uniqueOldSpecs = new Map<string, typeof oldSpecTypes[0]>();
  ...
  for (const [, oldSpec] of uniqueOldSpecs) {
    ...
    try {
      ...
      const alreadyExists = existing.some(
        (e) => e.primaryName.toLowerCase() === oldSpec.specTypeName.toLowerCase()
      );
      ...
      const enriched = await enrichOldSpecType(
        oldSpec.specTypeName,
        oldSpec.units,
        oldSpec.valueType,
        oldSpec.description
      );
      ...
      if (validation.isDuplicate && validation.similarity >= 95) {
        console.log(
          `  ✗ Duplicate of "${validation.conflictingSpecType}" (${validation.similarity}%)`
        );
        skippedDuplicates++;
        continue;
      }
