import { SpecType, Domain, ValueType } from '../types/schemas.js';
...
async function generateProposal(
  ...
  oldSpecTypeNames: string[],
  avoidNames: string[] = []
): Promise<GeneratedProposal> {
  const existingContext =
    existingSpecTypes.length > 0
  ...
  const oldSpecContext =
    oldSpecTypeNames.length > 0
...
Return ONLY a JSON object:
{
  ...
  return await askClaudeJSON<GeneratedProposal>(prompt, systemPrompt);
}

/**
 * Cleanup malformed response using AI
 */
async function cleanupMalformedResponse(
  malformedResponse: any,
  systemPrompt: string
): Promise<GeneratedSpecType> {
...
Return ONLY the corrected JSON, no explanations.`;
  ...
  return await askClaudeJSON<GeneratedSpecType>(cleanupPrompt, systemPrompt);
}

/**
 * Generate full spec type
 */
async function generateFullSpecType(
...
Return JSON matching the GeneratedSpecType interface with "primaryName" NOT "name".`;
  ...
  let response: GeneratedSpecType;
  
  try {
    response = await askClaudeJSON<GeneratedSpecType>(prompt, systemPrompt);
  } catch (error) {
 ...
 */
async function generateSingleSpecType(
  index: number,
  total: number,
  domain: Domain,
  category: string,
  ...
  existingSpecTypes: SpecType[],
  oldSpecTypeNames: string[],
  categoryRejectedNames: string[] = [] // Names rejected anywhere in this category
): Promise<SpecType | null> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const proposal = await generateProposal(
        ...
        existingSpecTypes,
        oldSpecTypeNames,
        categoryRejectedNames // Use persistent list
      );
      ...
      const specType = await generateFullSpecType(
        proposal,
        valueType,
        category,
        existingSpecTypes
 ...
 */
export async function generateGlobalSpecTypes(): Promise<SpecType[]> {
  ...
  const oldSpecTypes = loadOldSpecTypes(
    join(process.cwd(), 'SpecTypesOldDONTUSETHISONETOOBIGTOREAD.csv')
  );
  ...
  const valueTypes: ValueType[] = ['NUMERIC', 'SELECT', 'MULTI_SELECT', 'RANGE', 'BOOLEAN'];
  ...
  for (const [domain, cats] of Object.entries(categories)) {
    ...
    for (const category of cats) {
      ...
      const categoryRejectedNames: string[] = []; // Persist across all attempts in this category
      ...
      while (consecutiveDuplicates < EXHAUSTION_THRESHOLD && categoryCount < MAX_PER_CATEGORY) {
        ...
        totalGenerated++;
        const specType = await generateSingleSpecType(
          totalGenerated,
          0, // No fixed total - show 0 for unlimited
          domain as Domain,
          category,
          valueType,
          allExisting,
          oldSpecTypeNames,
          categoryRejectedNames // Pass persistent rejected list
        );
