...
Return ONLY a JSON object:
{
  ...
  const response = await askClaudeJSON<GeneratedProposal>(prompt, systemPrompt);
  return response;
}
  ...
  const existingContext =
    existingSpecTypes.length > 0
      ? `\n\nEXISTING SPEC TYPES:\n${existingSpecTypes
          ...
          .map(
            (s) =>
              `- ${s.primaryName} (${s.domain}): ${s.description.substring(0, 80)}...`
          )
          .join('\n')}`
      : '';
...
5. ${
    targetValueType === 'SELECT' || targetValueType === 'MULTI_SELECT'
...
6. ${
    targetValueType === 'NUMERIC' || targetValueType === 'RANGE'
...
Return a JSON object with the structure provided.`;
  ...
  const response = await askClaudeJSON<GeneratedSpecType>(prompt, systemPrompt);
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (validation.isDuplicate) {
        console.log(
          `  ✗ Rejected (${validation.similarity}% similar to "${validation.conflictingSpecType}")`
        );
 ...
 */
export async function generateFullSpecTypes(): Promise<SpecType[]> {
  ...
  const domains: Domain[] = ['HVAC', 'ELECTRICAL', 'PLUMBING', 'FIRE_PROTECTION'];
 ...
 */
function determineDomain(componentName: string): Domain {
