  ...
  const existingContext =
    existingSpecTypes.length > 0
      ? `\n\nEXISTING SPEC TYPES:\n${existingSpecTypes
          .map(
            (s) =>
              `- ${s.primaryName} (${s.domain}): ${s.description.substring(0, 80)}... | Alternates: ${s.alternateNames.slice(0, 3).join(', ')}`
          )
          .join('\n')}`
      : '';
    ...
    try {
      ...
      if (validation.isDuplicate) {
        ...
        if (attempt < maxRetries) {
          ...
          console.log(
            `   ✗ Max retries reached. Skipping this spec type.`
          );
          return null;
        }
      }
 ...
 */
export async function generateTestSpecTypes(): Promise<SpecType[]> {
  ...
  const targets: Array<{ domain: Domain; valueType: ValueType }> = [
