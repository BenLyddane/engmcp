...
import { SpecType, Unit } from '../types/schemas.js';
 ...
 */
function findUnitBySymbol(symbol: string, units: Unit[]): Unit | null {
  if (!symbol) return null;
  
  return units.find(u => 
    u.symbol === symbol || 
    u.symbol.toLowerCase() === symbol.toLowerCase() ||
    u.abbreviations.some(abbr => 
      abbr === symbol || 
      abbr.toLowerCase() === symbol.toLowerCase()
    )
  ) || null;
}
