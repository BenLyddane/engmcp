 ...
 */
async function batchDetectDuplicates(
  candidates: Array<{ spec1: SpecType; spec2: SpecType; stringSimilarity: number }>,
...
[
  {
    ...
    try {
      const results = await askClaudeJSON<Array<{
        pairIndex: number;
        areDuplicates: boolean;
        similarity: number;
        reason: string;
      }>>(prompt, 'You are an expert at identifying duplicate MEP specifications.');
...
async function findSemanticDuplicates(
  ...
  const byDomain: Record<string, SpecType[]> = {};
  ...
  const candidates: Array<{ spec1: SpecType; spec2: SpecType; stringSimilarity: number }> = [];
