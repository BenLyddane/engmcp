...
interface Checkpoint {
  ...
  groupsClassified: Record<string, string[]>;
  conversionsGroupsCompleted: string[];
  currentConversionGroupId: string | null;
  unitsCompleted: string[];
  lastUpdated: string;
}

async function classifyUnit(unit: Unit, existingGroups: string[]): Promise<string> {
  const prompt = `Classify this MEP unit: ${unit.symbol}
EXISTING GROUPS: ${existingGroups.join(', ')}
Return JSON: {"groupName": "exact name"}`;
  
  try {
    const response = await askClaudeJSON<{ groupName: string }>(prompt, 'MEP expert');
...
function validateConversion(conv: ConversionData, fromUnit: Unit, allUnits: Unit[]): ConversionData | null {
  if (!conv.from || !conv.to || typeof conv.multiplier !== 'number' || !conv.equation) return null;
  if (conv.from !== fromUnit.symbol || !allUnits.find(u => u.symbol === conv.to)) return null;
  if (conv.multiplier <= 0 || !isFinite(conv.multiplier) || !conv.equation.includes('x')) return null;
  return { from: conv.from, to: conv.to, multiplier: conv.multiplier, equation: conv.equation.trim() };
}

async function generateConversionsFromUnit(fromUnit: Unit, toUnits: Unit[], groupName: string): Promise<ConversionData[]> {
...
Return ${toUnits.length} conversions: [{"from":"${fromUnit.symbol}","to":"unit","multiplier":N,"equation":"x * N"}]`;
  ...
  try {
    return await askClaudeJSON<ConversionData[]>(prompt, 'MEP conversion expert');
  } catch {
    return [];
  }
}
...
function loadCheckpoint(): Checkpoint {
  ...
  if (existsSync(path)) {
    try {
      ...
      if (data.classificationComplete !== undefined) return data;
    } catch {}
  }
...
function saveData(units: Unit[], groups: UnitGroup[]): void {
...
async function main() {
  ...
  if (batchGroupLimit) console.log('Batch Mode: ' + batchGroupLimit + ' groups max');
  console.log('');

  const data = JSON.parse(readFileSync(join(process.cwd(), 'output', 'global-units-master.json'), 'utf-8'));
  const allUnits: Unit[] = data.units;
  const unitGroups: UnitGroup[] = data.unitGroups;
  ...
  if (!checkpoint.classificationComplete && unmapped.length > 0) {
    ...
    for (let i = 0; i < unmapped.length; i += batchSize) {
      ...
      batch.forEach((unit, idx) => {
        if (!checkpoint.groupsClassified[groupNames[idx]]) checkpoint.groupsClassified[groupNames[idx]] = [];
  ...
  for (const [groupName, unitIds] of Object.entries(checkpoint.groupsClassified)) {
    ...
    for (const unitId of unitIds) {
      ...
      if (unit) unit.unitGroupId = groupId;
    }
  }

  const groupsNeedingConversions = unitGroups.filter(g => {
    if (checkpoint.conversionsGroupsCompleted.includes(g.id)) return false;
  ...
  for (const [idx, group] of groupsNeedingConversions.entries()) {
    ...
    for (let i = 0; i < unitsToProcess.length; i += 20) {
      ...
      const promises = batch.map(async fromUnit => {
        ...
        const valid = conversions.map(c => validateConversion(c, fromUnit, groupUnits)).filter(Boolean) as ConversionData[];
      ...
      for (const { fromUnit, conversions } of results) {
        for (const convData of conversions) {
          ...
          if (!toUnit || group.conversions.some(c => c.fromUnitId === fromUnit.id && c.toUnitId === toUnit.id)) continue;
