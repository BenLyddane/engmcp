 ...
 */
export async function validateSpecTypes(specTypes) {
    const report = {
    ...
    for (const spec of specTypes) {
        if (spec.alternateNames.length === 0) {
            ...
            report.warnings.push(`Spec type "${spec.primaryName}" has no alternate names`);
        }
    }
    ...
    for (const spec of specTypes) {
        if (spec.notNames.length === 0) {
            ...
            report.warnings.push(`Spec type "${spec.primaryName}" has no "not names" for disambiguation`);
        }
    }
    // Validate value types
    for (const spec of specTypes) {
        if ((spec.valueType === 'SELECT' || spec.valueType === 'MULTI_SELECT') &&
            (!spec.valueOptions || spec.valueOptions.length === 0)) {
            report.invalidValueTypes.push(spec.primaryName);
            report.errors.push(`Spec type "${spec.primaryName}" is ${spec.valueType} but has no value options`);
        }
        if (spec.valueType === 'NUMERIC' &&
            spec.minValue !== undefined &&
            spec.maxValue !== undefined &&
            spec.minValue > spec.maxValue) {
            report.errors.push(`Spec type "${spec.primaryName}" has minValue > maxValue`);
        }
    }
 ...
 */
async function detectSemanticDuplicates(specTypes) {
    const duplicates = [];
    // Compare all pairs
...
Consider them duplicates if they represent the same underlying physical property, even if the names differ slightly.`;
            try {
                const response = await askClaude(prompt, 'You are an expert in HVAC, electrical, plumbing, and fire protection systems. Your task is to identify if two specification types are semantically equivalent.');
                // Parse the response
                ...
                console.error(`Error comparing ${spec1.primaryName} and ${spec2.primaryName}:`, error);
            }
        }
    }
    return duplicates;
...
export function printValidationReport(report) {
    ...
    if (report.semanticDuplicates.length > 0) {
        ...
        for (const dup of report.semanticDuplicates) {
            console.log(`  - "${dup.specType1}" and "${dup.specType2}" (${dup.similarity}% similar)`);
