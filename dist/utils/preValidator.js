 ...
 */
export async function preValidateProposal(proposal, existingSpecTypes) {
    ...
    const exactMatch = existingSpecTypes.find((spec) => spec.primaryName && spec.primaryName.toLowerCase() === proposal.primaryName.toLowerCase());
    if (exactMatch) {
        return {
            isUnique: false,
            isDuplicate: true,
    ...
    const existingContext = existingSpecTypes
        .map((spec) => `- ${spec.primaryName} (${spec.domain}): ${spec.description.substring(0, 100)}${spec.description.length > 100 ? '...' : ''}`)
        .join('\n');
...
Respond with ONLY a JSON object:
{
  ...
  "reason": "brief explanation"
    ...
    try {
        const response = await askClaude(prompt, 'You are an expert in HVAC, electrical, plumbing, and fire protection specification types. You excel at identifying semantic duplicates.');
...
//# sourceMappingURL=preValidator.js.map