#!/usr/bin/env node
import { askClaude, askClaudeJSON } from '../utils/claude.js';
 ...
 */
async function matchBatch(component, specTypeBatch, retryCount = 3) {
  ...
  ...
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
        try {
            let results;
            try {
                results = await askClaudeJSON(prompt, 'You are an expert in MEP equipment specifications.');
            }
            catch (parseError) {
...
async function matchComponentsToSpecs() {
    ...
    console.log('╚════════════════════════════════════════════════════════╝\n');
    const allComponents = loadComponentTypes(join(process.cwd(), 'ComponentTypesFullDataDontRead.csv'));
    ...
    for (let i = 0; i < componentsToProcess.length; i += concurrency) {
        ...
        const saveCheckpointFn = (mappings) => {
            const checkpointPath = join(process.cwd(), 'output', 'mappings-checkpoint.json');
            const checkpoint = {
                mappings,
                progress: {
        ...
        const promises = chunk.map((component, idx) => matchComponentToAllSpecs(component, specTypes, i + idx, components.length, allMappings, saveCheckpointFn));
...
//# sourceMappingURL=componentSpecMatcher.js.map