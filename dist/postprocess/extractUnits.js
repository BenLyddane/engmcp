...
function extractUnits() {
    ...
    for (const unit of sortedUnits) {
        ...
        else if (['gpm', 'l/min', 'l/s', 'cfm', 'm³/h', 'm3/h', 'gal/min'].includes(unitLower)) {
 ...
 */
function printReport(extracted) {
    console.log('═══════════════════════════════════════');
    ...
    topUnits.forEach((usage, index) => {
        console.log(`   ${index + 1}. ${usage.unit} (${usage.count} occurrences in ${usage.specTypes.length} spec types)`);
    });
...
main();
//# sourceMappingURL=extractUnits.js.map