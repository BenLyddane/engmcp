 ...
 */
function generateCompleteConversions(units, existingConversions) {
    ...
    for (const fromUnit of units) {
        for (const toUnit of units) {
            ...
            if (multiplier !== undefined) {
                ...
                const existing = existingConversions.find(c => c.fromUnitId === fromUnit.id && c.toUnitId === toUnit.id);
                if (existing) {
...
function validateConversions(conversions) {
    ...
    for (const conv of conversions) {
        const reverse = conversions.find(c => c.fromUnitId === conv.toUnitId && c.toUnitId === conv.fromUnitId);
        if (reverse) {
...
async function ensureCompleteConversions() {
    ...
    const unitGroups = unitsData.unitGroups;
...
//# sourceMappingURL=ensureCompleteConversions.js.map